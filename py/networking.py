...
# Named tuples for sending more deliberate messages over the network
# There's got to be a better solution than putting these at global scope in networking.py
# If I make them part of the Networking class, pickling fails
BcastPin = namedtuple('BcastPin', ['pin', 'brightness', 'use_overrides'])
BcastArray = namedtuple('BcastArray', ['array', 'use_overrides'])
BcastOverrides = namedtuple('BcastOverrides', ['always_off', 'always_on', 'inverted'])
...
class Networking(object):
    ...
    def __init__(self, cm):
        ...
        self.get_overrides = cm.network.get_overrides
        self.playing = False
        # ready is added to make sure networking has actually been set up before we broadcast
        self.ready = False
