...
class Gui(Canvas):
    ...
    def __init__(self, parent):
        ...
        self.channel_keys = range(len(CM.network.channels))
    ...
    def tkinter_function(self):
        ...
        if isinstance(data[0], int):
            pin = data[0]
            for x in self.channel_keys:
                if pin == abs(self.network.channels[x]):
                    # Generally these are not music related, so let's ignore the overrides
                    self.set_light(x, False, float(data[1] if self.network.channels[pin] >= 0 else 0))
            done = True
            ...
            for x in self.channel_keys:
                if pin == abs(self.network.channels[x]):
                    self.set_light(x, data[0].use_overrides, float(data[0].brightness if self.network.channels[pin] >= 0 else 0))
            done = True

        elif isinstance(data[0], networking.BcastOverrides):
            #TO DO - Add network custom overrides to simulator
        ...
        if not done:
            for pin in self.channel_keys:
                self.set_light(pin, temp_override, b_levels[self.network.channels[pin]] if self.network.channels[pin] >= 0 else 0)
